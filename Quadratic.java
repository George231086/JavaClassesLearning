...
public class Quadratic {
	...
	static String[] parseQuad(String input) {
		...
		coeffs[2] = (c.endsWith("+") || c.endsWith("-")) ? c + "1" : c;
	...
	public static void main(String args[]) {
		...
		try {
			...
			System.out.println("Either input is not a quadratic or in the form ax^2+bx+c!");
			System.exit(1);
		} finally {
			scan.close();
		}
		...
		if (quad.hasRealSolutions()) {

			if (quad.hasDoubleRoot())
				System.out.printf("Double root! The solution to 2dp is %.2f\n", x1);
			else
				System.out.printf("The solutions to 2dp are:\ns1=%.2f and s2=%.2f\n",
						x1 + Math.sqrt(x2), x1 - Math.sqrt(x2));
		} else
			System.out.printf(
